...
if command -v netstat >/dev/null 2>&1; then
    PORT_STATUS=$(netstat -an 2>/dev/null | grep -q ":${MCP_PORT:-8202} " && echo "✅" || echo "❌")
    echo "端口监听: ${PORT_STATUS} ${MCP_PORT:-8202}"
elif command -v lsof >/dev/null 2>&1; then
    PORT_STATUS=$(lsof -i :${MCP_PORT:-8202} >/dev/null 2>&1 && echo "✅" || echo "❌")
    echo "端口监听: ${PORT_STATUS} ${MCP_PORT:-8202}"
else
